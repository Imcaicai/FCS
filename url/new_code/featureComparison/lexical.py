...
dataset_path = '../data/x_train.csv'
abnormal_dataset_path = '../data/x_train_abnormal.csv'
test_path = '../data/test.csv'
lexical_train_path = 'lexical_train.csv'
lexical_train_abnormal_path = 'lexical_train_abnormal.csv'
lexical_test_path = 'lexical_test.csv'
...
def get_train_set():
    ...
    lexical_feature = np.array((deli_num, hyp_num, url_len, nor_tld_token, sus_word_token, ip_in_hostname)).T

    # 保存 lexical feature set
    np.save('lexical_train.npy', lexical_feature)
    lexical = pd.DataFrame(lexical_feature,
                           columns=['deli_num', 'hyp_num', 'url_len', 'nor_tld_token', 'sus_word_token',
                                    'ip_in_hostname'])
    lexical.to_csv(lexical_train_path, index=False)

def get_train_abnormal_set():
    # 读取数据集
    ...
    # 保存 lexical feature set
    np.save('lexical_train_abnormal.npy', lexical_feature)
    lexical = pd.DataFrame(lexical_feature,
                           columns=['deli_num', 'hyp_num', 'url_len', 'nor_tld_token', 'sus_word_token',
                                    'ip_in_hostname'])
    lexical.to_csv(lexical_train_abnormal_path, index=False)
...
def get_test_set():
    ...
    # 保存 lexical feature set
    np.save('lexical_test.npy', lexical_feature)
    lexical = pd.DataFrame(lexical_feature,
                           columns=['deli_num', 'hyp_num', 'url_len', 'nor_tld_token', 'sus_word_token',
                                    'ip_in_hostname'])
    lexical.to_csv(lexical_test_path, index=False)

if __name__ == '__main__':
    get_train_set()
    get_train_abnormal_set()
    get_test_set()
