...
from transformers import (
    ...
    DistilBertForSequenceClassification, DistilBertTokenizer, AlbertConfig,
...
class urlProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        with open(os.path.join(data_dir, 'abnormal_train.csv'), 'r') as f:
            return self._create_examples(list(csv.reader(f)), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        import csv
        with open(os.path.join(data_dir, 'abnormal_test.csv'), 'r') as f:
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            guid = "%s-%s" % (set_type, i)
            # YZY:replace special token with comma
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        # !YZY: There exists something about task's name.
        if eval_task in ["cnews", "url_classification"]:
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    cached_features_file = os.path.join(
        args.data_dir,
        'cached_{}_{}_{}_{}'.format('dev' if evaluate else 'train', 'bert',
...
def main():
    ...
    ## Required parameters
    # YZY:maybe we should change input file's structure
    parser.add_argument(
        "--data_dir",
        default='./url',
        type=str,
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            prefix = "checkpoint-" + str(checkpoint.split(
